...
mod_plants_img_quiz_ui <- function(id) {
  ns <- NS(id)
  tagList(
    layout_columns(
      col_widths = c(4, 8),
...
#' plants_img_quiz Server Functions
#'
#' @noRd
mod_plants_img_quiz_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    # generate randomized integer between 1 and length(unique(plants$species_german)
    random_species_number <- sample(1:length(unique(plants$species_german)), 1)

    # browser()
    # unique_species_german <- unique(plants$species_german)  # Assuming this is your species list
    values <- reactiveValues(current_species = plants$species_german[random_species_number], species= plants$species[random_species_number])
    random_specs <- reactiveValues(data=NULL)

    observe({
      req(values$current_species)
      # browser()
      random_specs$data <- plants %>%
    ...
    observeEvent(values$feedback, {
      if (values$feedback == "Richtig! Weiter zum nächsten Bild." || values$feedback == paste("Lösung: ", values$current_species)) {
        ...
        values$species <- plants$species[next_index]
