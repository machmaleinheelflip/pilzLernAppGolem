#' hymenoptera_img_quiz UI Function
#'
...
mod_hymenoptera_img_quiz_ui <- function(id) {
  ns <- NS(id)
  tagList(
    layout_columns(
      col_widths = c(4, 8),
...
#' hymenoptera_img_quiz Server Functions
#'
#' @noRd
mod_hymenoptera_img_quiz_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    # generate randomized integer between 1 and length(unique(hymenoptera$species_german)
    random_species_number <- sample(1:length(unique(hymenoptera$species_german)), 1)

    # browser()
    # unique_species_german <- unique(hymenoptera$species_german)  # Assuming this is your species list
    values <- reactiveValues(current_species = hymenoptera$species_german[random_species_number], species= hymenoptera$species[random_species_number])
    random_specs <- reactiveValues(data=NULL)

    observe({
      req(values$current_species)
      # browser()
      random_specs$data <- hymenoptera %>%
