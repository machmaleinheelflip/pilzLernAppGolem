...
process_species <- function(speciesKey) {
  # Load existing data
  if (file.exists("data-raw/data_6.rds")) {
    old_data <- readRDS("data-raw/data_6.rds")
  ...
  saveRDS(new_data, "data-raw/data_6.rds")
}
...
pbapply::pblapply(species_list$speciesKey[332:2500], process_species)
...
shrooms <- readRDS("data-raw/data_6.rds")
usethis::use_data(shrooms, overwrite = TRUE)
